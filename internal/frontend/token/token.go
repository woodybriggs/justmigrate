...
const (
	...
	TokenKindOffset_Misc     TokenKind = 400
	TokenKindOffset_Keywords TokenKind = 500
)
...
const (
	TokenKind_neq TokenKind = iota + 1 + TokenKindOffset_Misc
	TokenKind_gte
	TokenKind_lte
)
...
const (
	...
	TokenKind_Keyword_COLUMN
)

const (
	Keyword_DROP          string = "drop"
	Keyword_ADD           string = "add"
	Keyword_ALTER         string = "alter"
...
var KeywordIndex = NewIndex[string, TokenKind]().
	Add(Keyword_DROP, TokenKind_Keyword_DROP).
	Add(Keyword_ADD, TokenKind_Keyword_ADD).
	Add(Keyword_ALTER, TokenKind_Keyword_ALTER).
	...
	Add(Keyword_COLUMN, TokenKind_Keyword_COLUMN)
...
func Keyword(kword string) Token {
	...
	if !ok {
		panic(fmt.Sprintln("invalid keyword", kword))
	}
	return Token{
		Text: kword,
		Kind: kind,
...
const (
	...
	CostMid      Cost = 10
	CostHigh     Cost = 50
	CostProhibit Cost = 100
)

func (t Token) InsertionCost() Cost {
	switch t.Kind {
	case ',', ';', ')':
		return CostLow
	case '(':
...
func (t Token) DeletionCost() Cost {
	switch t.Kind {
	case ',', ';', ')':
		return CostLow
	case TokenKind_Keyword_CREATE,
